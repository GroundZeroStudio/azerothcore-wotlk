    ...
    struct bot_druid_ai : public bot_ai
    {
        ...
        bool removeShapeshiftForm() override
        {
            ...
            //if (form != FORM_NONE)
            {
                ...
                {
                    //LOG_ERROR("entities.player", "druid_bot::removeShapeshiftForm(): still has poweType {}!", uint32(me->getPowerType()));
                    me->SetPowerType(POWER_MANA);
                }
                if (me->GetShapeshiftForm() != FORM_NONE)
                {
                    //LOG_ERROR("entities.player", "druid_bot::removeShapeshiftForm(): still speshifted into {}!", uint32(me->GetShapeshiftForm()));
        ...
        void Attack(uint32 diff)
        {
                ...
                (IsSpellReady(NATURES_SWIFTNESS_1, diff, false) && doCast(me, GetSpell(NATURES_SWIFTNESS_1)))))
            {
                ...
                {
                    Position pos;
                    opponent->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, 15.f, 0.0f, opponent->GetAbsoluteAngle(me));
        ...
        void setStats(BotStances form)
        {
            ...
            switch (form)
            {
                case DRUID_BEAR_FORM:
                    ...
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_RAGE");
                ...
                case DRUID_CAT_FORM:
                    ...
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_ENERGY");
                ...
                case DRUID_MOONKIN_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (moonkin)");
                        me->SetPowerType(POWER_MANA);
                    }
                ...
                case DRUID_TREE_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (tree)");
                        me->SetPowerType(POWER_MANA);
                    }
                ...
                case DRUID_TRAVEL_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (travel)");
                        me->SetPowerType(POWER_MANA);
                    }
                    break;
                case DRUID_AQUATIC_FORM:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (aquatic)");
                        me->SetPowerType(POWER_MANA);
                    }
                    break;
                ...
                //    if (me->getPowerType() != POWER_MANA)
                //    {
                //        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (flight)");
                //        me->SetPowerType(POWER_MANA);
                //    }
                //    break;
                case BOT_STANCE_NONE:
                    if (me->getPowerType() != POWER_MANA)
                    {
                        //LOG_ERROR("entities.player", "druid_bot::setStats(): has to set powerType to POWER_MANA (deshape)");
                        me->SetPowerType(POWER_MANA);
                    }
                    break;
                default:
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            if (_form == DRUID_CAT_FORM)
            {
                    ...
                    baseId == RAKE_1 || baseId == RAVAGE_1 || baseId == SHRED_1)
                {
                    comboPoints++;
                    //debug
                    //LOG_ERROR("entities.player", "druid_bot CP GEN: %s adds 1, now {}", spell->SpellName[0], uint32(comboPoints));
                    if (primalFuryProc)
                    {
                        comboPoints++;
                        //debug
                        //LOG_ERROR("entities.player", "druid_bot CP EX: now {}", uint32(comboPoints));
                    }
                    if (comboPoints > 5)
                    {
                        comboPoints = 5;
                        //debug
                        //LOG_ERROR("entities.player", "druid_bot CP NOR: now {}", uint32(comboPoints));
                    }
                }
                ...
                {
                    //debug
                    //LOG_ERROR("entities.player", "druid_bot CP SPEND1: {} to 0", uint32(comboPoints));
                    if (lvl >= 25 && comboPoints > 0)
                    {
                        if (urand(1,100) <= uint32(comboPoints * 20))
                        {
                            ...
                            //debug
                            //LOG_ERROR("entities.player", "druid_bot CP SPEND1: PS proc!");
                        }
                    }
                    comboPoints = 0;
                }
