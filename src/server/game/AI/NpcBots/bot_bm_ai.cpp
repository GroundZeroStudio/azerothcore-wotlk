    ...
    public:
        ...
        void IllusionsCheck(uint32 diff)
        {
            ...
            for (std::list<ObjectGuid>::const_iterator itr = _illusionGuids.begin(); itr != _illusionGuids.end(); ++itr)
            {
                ...
                if (!ill)
                {
                    LOG_ERROR("entities.player", "bm_bot::IllusionsCheck(): unit {} is not found in world!", (*itr).GetCounter());
                    continue;
                }
        ...
        void MirrorImageFinish()
        {
            ...
            {
                Creature* illusion = me->SummonCreature(NPC_MIRROR_IMAGE_BOT, *me, TEMPSUMMON_MANUAL_DESPAWN);
                if (!illusion)
                    continue;
        ...
        void CriticalStrikeFinish(ObjectGuid targetGuid, CalcDamageInfo* calcdinfo, bool /*windwalk*/)
        {
            ...
            SpellInfo const* pSpellInfo = sSpellMgr->GetSpellInfo(CRITICAL_STRIKE_1);
            SpellNonMeleeDamage snmd(me, target, pSpellInfo, SPELL_SCHOOL_MASK_NORMAL);
            snmd.damage = dinfo.GetDamage() + dinfo.GetAbsorb() + dinfo.GetResist() + dinfo.GetBlock();
            snmd.absorb = dinfo.GetAbsorb();
            snmd.resist = dinfo.GetResist();
            snmd.blocked = dinfo.GetBlock();
            snmd.physicalLog = true;
            me->SendSpellNonMeleeDamageLog(&snmd);
