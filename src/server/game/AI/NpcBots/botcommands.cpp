...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "raid", SEC_GAMEMASTER, false, HandleNpcBotDebugRaidCommand, "" },
            { "mount", SEC_GAMEMASTER, false, HandleNpcBotDebugMountCommand, ""  },
            { "spellvisual", SEC_GAMEMASTER, false, HandleNpcBotDebugSpellVisualCommand, ""  },
            { "states", SEC_GAMEMASTER, false, HandleNpcBotDebugStatesCommand, ""  },
        };

        static std::vector<ChatCommand> npcbotSetCommandTable =
        {
            { "faction", SEC_GAMEMASTER, false, HandleNpcBotSetFactionCommand, ""  },
            { "owner", SEC_GAMEMASTER, false, HandleNpcBotSetOwnerCommand, ""  },
            { "spec", SEC_GAMEMASTER, false, HandleNpcBotSetSpecCommand, ""  },
        };

        static std::vector<ChatCommand> npcbotCommandCommandTable =
        {
            { "standstill", SEC_PLAYER, false, HandleNpcBotCommandStandstillCommand, ""  },
            { "stopfully",  SEC_PLAYER, false, HandleNpcBotCommandStopfullyCommand, ""  },
            { "follow", SEC_PLAYER, false, HandleNpcBotCommandFollowCommand, ""  },
        };

        static std::vector<ChatCommand> npcbotAttackDistanceCommandTable =
        {
            { "short", SEC_PLAYER, false, HandleNpcBotAttackDistanceShortCommand, ""  },
            { "long", SEC_PLAYER, false, HandleNpcBotAttackDistanceLongCommand, ""  },
            { "", SEC_PLAYER, false, HandleNpcBotAttackDistanceExactCommand, ""  },
        };

        static std::vector<ChatCommand> npcbotDistanceCommandTable =
        {
            { "attack", SEC_MODERATOR, false, nullptr, "", npcbotAttackDistanceCommandTable },
            { "", SEC_PLAYER, false, HandleNpcBotFollowDistanceCommand, "" },
        };

        static std::vector<ChatCommand> npcbotOrderCommandTable =
        {
            { "cast", SEC_PLAYER, false, HandleNpcBotOrderCastCommand, ""  },
        };

        static std::vector<ChatCommand> npcbotVehicleCommandTable =
        {
            { "eject", SEC_PLAYER, false, HandleNpcBotVehicleEjectCommand, ""  },
        };

        static std::vector<ChatCommand> npcbotDumpCommandTable =
        {
            { "load", SEC_ADMINISTRATOR, false, HandleNpcBotDumpLoadCommand, "" },
            { "write", SEC_ADMINISTRATOR, false, HandleNpcBotDumpWriteCommand, "" },
        };
        ...
        static std::vector<ChatCommand> npcbotCommandTable =
        {
            ...
            { "set", SEC_MODERATOR, false, nullptr, "", npcbotSetCommandTable},
            { "add", SEC_GAMEMASTER, false, HandleNpcBotAddCommand, "" },
            { "remove", SEC_GAMEMASTER, false, HandleNpcBotRemoveCommand, ""  },
            { "spawn", SEC_GAMEMASTER, true, HandleNpcBotSpawnCommand, ""},
            { "move", SEC_GAMEMASTER, false, HandleNpcBotMoveCommand, ""  },
            { "delete", SEC_GAMEMASTER, false, HandleNpcBotDeleteCommand, ""  },
            { "lookup", SEC_GAMEMASTER, false, HandleNpcBotLookupCommand, ""  },
            { "revive", SEC_GAMEMASTER, false, HandleNpcBotReviveCommand, ""  },
            { "reloadconfig", SEC_GAMEMASTER, false, HandleNpcBotReloadConfigCommand, "" },
            { "command", SEC_MODERATOR, false, nullptr, "", npcbotCommandCommandTable},
            { "info", SEC_PLAYER, false, HandleNpcBotInfoCommand, ""  },
            { "hide", SEC_PLAYER, false, HandleNpcBotHideCommand, ""  },
            { "unhide", SEC_PLAYER, false, HandleNpcBotUnhideCommand, ""  },
            { "show", SEC_PLAYER, false, HandleNpcBotUnhideCommand, ""  },
            { "recall", SEC_PLAYER, false, HandleNpcBotRecallCommand, ""  },
            { "kill", SEC_PLAYER, false, HandleNpcBotKillCommand, ""  },
            { "suicide", SEC_PLAYER, false, HandleNpcBotKillCommand, ""  },
            { "distance", SEC_MODERATOR, false, nullptr, "", npcbotDistanceCommandTable},
            { "order", SEC_MODERATOR, false, nullptr, "", npcbotOrderCommandTable},
            { "vehicle", SEC_MODERATOR, false, nullptr, "", npcbotVehicleCommandTable},
            { "dump", SEC_MODERATOR, false, nullptr, "", npcbotDumpCommandTable},
        };

        static std::vector<ChatCommand> commandTable =
        {
            {"npcbot", SEC_PLAYER, true, nullptr, "", npcbotCommandTable},
        };
        return commandTable;
    }
    ...
    static bool HandleNpcBotHideCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!owner->HaveBot())
        {
            ...
            handler->SendSysMessage("让你的机器人暂离消失");
        ...
        if (!owner->IsAlive())
        {
            handler->GetSession()->SendNotification("你已经挂了");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->SendSysMessage("机器人已经暂离消失");
        return true;
    }
    ...
    static bool HandleNpcBotUnhideCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!owner->HaveBot())
        {
            ...
            handler->SendSysMessage("召唤出你暂离消失的机器人");
            //handler->SendSysMessage("You have no bots!");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->SendSysMessage("机器人已经重新召唤");
        return true;
    }
    ...
    static bool HandleNpcBotKillCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        {
            handler->SendSysMessage("杀死机器人命令");
            handler->SendSysMessage("选择一个机器人会将选中的机器人杀死,如果选中的目标是自己,将把所有机器人都杀死");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->SendSysMessage("你必须选中一个机器人,或者选中自己");
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleNpcBotSetFactionCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            handler->SendSysMessage("该机器人已经有主人了,不能添加招募");
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    static bool HandleNpcBotSetOwnerCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (guidlow)
            found = ObjectAccessor::FindPlayerByLowGUID(guidlow);
        else
            found = ObjectAccessor::FindPlayerByName(characterName);
        ...
        guidlow = found->GetGUID().GetCounter();
    ...
    static bool HandleNpcBotLookupCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (CreatureTemplateContainer::const_iterator itr = ctc->begin(); itr != ctc->end(); ++itr)
        {
            ...
            //LOG_ERROR("entities.unit", "NpcBotLookup: cur %u", id);

            NpcBotExtras const* _botExtras = BotDataMgr::SelectNpcBotExtras(id);
            if (!_botExtras)
                continue;

            //LOG_ERROR("entities.unit", "NpcBotLookup: found extras...");

            if (_botExtras->bclass != botclass)
                continue;

            //LOG_ERROR("entities.unit", "NpcBotLookup: class matches...");
            ...
            //LOG_ERROR("entities.unit", "NpcBotLookup: ading to list");
    ...
    static bool HandleNpcBotSpawnCommand(ChatHandler* handler, const char* args)
    {
        ...
        stmt->SetData(1, id);
        stmt->SetData(2, id);
        auto res2 = WorldDatabase.Query(stmt);
        if (res2)
        {
        ...
        uint32_t nPhaseMask = chr->GetPhaseMaskForSpawn();
        Map* map = chr->GetMap();
        ...
        if (!creature->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, nPhaseMask, id, 0, x, y, z, o))
        ...
        float z   = 24.4919f;
        float o   = 2.91242f;
        Map*  map = sMapMgr->FindBaseMap(0);
        uint32_t nPhaseMask = PHASEMASK_NORMAL;
        ...
        creature->SaveToDB(map->GetId(), (1 << map->GetSpawnMode()), nPhaseMask);
    ...
    static bool HandleNpcBotReviveCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!u)
        {
            handler->SendSysMessage("机器人复活命令");
            handler->SendSysMessage("选定一个机器人会复活选定机器人,如果选定的目标是自己将复活所有机器人");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (Player* master = u->ToPlayer())
        {
            if (!master->HaveBot())
            {
                handler->PSendSysMessage("%s 他不是机器人!", master->GetName().c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            master->GetBotMgr()->ReviveAllBots();
            handler->SendSysMessage("机器人复活了");
            return true;
        }
        else if (Creature* bot = u->ToCreature())
        {
            if (bot->GetBotAI())
            {
                if (bot->IsAlive())
                {
                    handler->PSendSysMessage("%s 他还没有死", bot->GetName().c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }
        ...
        handler->SendSysMessage("你必须选中一个机器人");
        handler->SetSentErrorMessage(true);
        return false;
    }
    ...
    static bool HandleNpcBotAddCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!cre || cre->GetTypeId() != TYPEID_UNIT)
        {
            ...
            handler->SendSysMessage("允许雇用选定的不受控制的机器人");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->PSendSysMessage("%s 现在是你的机器人了", bot->GetName().c_str());
            return true;
        }

        handler->SendSysMessage("机器人招募失败,可能因为同职业数量已经最大了或者超过了招募数量的上限");
        handler->SetSentErrorMessage(true);
        return false;
    }
