...
//uint32 const DrinkSpells[MAX_FEAST_SPELLS][2] = //原始喝水恢复
//{
//{  1,   430 },
 //   {  5,   431 },
  //  { 15,   432 },
...
const std::string& bot_ai::LocalizedNpcText(Player const* forPlayer, uint32 textId)
{
    ...
    {
        LOG_ERROR("entities.player", "NPCBots: bot text string #{} is not localized, at least for %s",
...
void bot_ai::BotSay(const std::string &text, Player const* target) const
{
    ...
    me->Say(text.c_str(), LANG_UNIVERSAL);
}
...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    {
        LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): bot %s (id: {}) has master %s while trying to set to %s...",
            me->GetName().c_str(), me->GetEntry(), master->GetName().c_str(), newowner->GetName().c_str());
        return false;
    }
    if (!IAmFree())
    {
        LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): minion bot %s (id: {}) IS NOT FREE (has master %s) while trying to set to %s",
            me->GetName().c_str(), me->GetEntry(), master->GetName().c_str(), newowner->GetName().c_str());
        return false;
    }
    ...
    {
        //LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): player %s ({}) can't add bot %s (FATAL), removing...",
        ...
        if (_ownerGuid)
        {
            LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): %s's master %s (guid: {}) is found but bot failed to set owner (fatal)! Unbinding bot temporarily (until server restart)...",
...
void bot_ai::CheckOwnerExpiry()
{
    ...
    time_t lastLoginTime = result ? time_t(fields[0].Get<uint32>()) : timeNow;

    //either expired or owner does not exist
    if (timeNow >= lastLoginTime + expireTime)
    {
        std::string name = "unknown";
        if (auto pPlayer = ObjectAccessor::FindPlayer(ownerGuid))
            name = pPlayer->GetName();

        LOG_DEBUG("server.loading", ">> %s's (guid: {}) ownership over bot %s ({}) has expired!",
        ...
        auto iiresult = CharacterDatabase.Query(stmt);
        if (iiresult)
        {
...
void bot_ai::ResetBotAI(uint8 resetType)
{
    ...
    {
        AbortTeleport();
        TeleportHomeEvent doTeleHomeEvent(this);
        doTeleHomeEvent.Execute(0, 0);
    }
    else
    {
        _atHome = false;
...
SpellCastResult bot_ai::CheckBotCast(Unit const* victim, uint32 spellId) const
{
        ...
        (victim->getLevel() + 10) < int32(spellInfo->GetFirstRankSpell()->SpellLevel))
    {
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            {
                //LOG_ERROR("scripts", "CheckBotCast(): deny cast of %s by %s on low-level target %s (lvl {})",
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    {
        LOG_ERROR("entities.player", "bot_ai::doCast(): %s (bot class {}) tried to cast spell {} on a dead target %s",
        ...
        !(flags & TRIGGERED_FULL_MASK) && (m_botSpellInfo->IsChanneled() || m_botSpellInfo->CalcCastTime()))
    {
        ...
        //    LOG_ERROR("entities.player", "bot_ai::doCast(): spell {} interrupts movement", spellId);
        me->BotStopMovement();
    }
    ...
    //debug
    //LOG_ERROR("entities.player", "CheckBotCast(): %s ({}) by %s on %s", m_botSpellInfo->SpellName[0], spellId, me->GetName().c_str(), victim->GetName().c_str());
...
/*
    ...
    if (result != SPELL_CAST_OK)
        LOG_ERROR("entities.player", "doCast(): %s ({}) by %s on %s failed with {}",
...
void bot_ai::_calculatePos(Position& pos) const
{
    ...
    if (bmover->CanFly())
    {
        ...
        for (uint8 i = 0; i != 5 + over; ++i)
        {
            ...
            mmover->GetNearPoint(bmover, x, y, z, mydist, 0.0f, angle);
...
{
    if (Group * gr = (IAmFree() ? nullptr : master->GetGroup()))
...
void bot_ai::BuffAndHealGroup(uint32 diff)
{
    ...
    if (!pGroup)
    {
        ...
        if (HasRole(BOT_ROLE_HEAL))
        {
            ...
            for (auto itr = master->m_Controlled.begin(); itr != master->m_Controlled.end(); ++itr)
            {
                Unit* u = *itr;
                if (!u->IsInWorld() || me->GetMap() != u->FindMap() || !u->IsAlive() || u->HasUnitState(UNIT_STATE_ISOLATED) ||
...
void bot_ai::RezGroup(uint32 REZZ)
{
    ...
    //LOG_ERROR("entities.player", "RezGroup by %s", me->GetName().c_str());
    ...
    //LOG_ERROR("entities.unit", "RezGroup: %s found {} targets", me->GetName().c_str(), uint32(targets.size()));
...
void bot_ai::CureGroup(uint32 cureSpell, uint32 diff)
{
    ...
    //LOG_ERROR("entities.player", "%s: CureGroup() on %s", me->GetName().c_str(), pTarget->GetName().c_str());
    std::list<Unit*> targets;
    ...
    else
    {
        ...
        for (GroupReference const* itr = pGroup->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            for (auto itr = tPlayer->m_Controlled.begin(); itr != tPlayer->m_Controlled.end(); ++itr)
            {
                Unit* u = *itr;
                if (!u || !u->IsPet() || !u->IsAlive() || me->GetDistance(u) > 30) continue;
...
void bot_ai::SetStats(bool force)
{
    ...
    /*LOG_ERROR("entities.player", "*etStats(): Updating bot %s, class: {}, race: {}, level {}, master: %s",
    ...
    if (force)
    {
        ...
        //[bing] 设置maxpower
        Powers myPower = me->getPowerType();
        if (!me->GetMaxPower(myPower))
            me->SetMaxPower(me->getPowerType(), me->GetCreatePowers(myPower));
...
bool bot_ai::IsInBotParty(ObjectGuid guid) const
{
    ...
    if (master->GetVehicle() && guid == master->GetCharmerGUID()) return true;
    if (me->GetVehicle() && guid == me->GetCharmerGUID()) return true;
    ...
    {
        if (master->GetPetGUID() == guid || (master->GetVehicle() && master->GetCharmerGUID() == guid))
            return true;
...
void bot_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 1)
    {
        LOG_ERROR("entities.player", "bot_ai::RefreshAura(): count is out of bounds (%i) for bot %s (botclass: {}, entry: {})",
    ...
    if (!spellId)
    {
        LOG_ERROR("entities.player", "bot_ai::RefreshAura(): spellId is 0 for bot %s (botclass: {}, entry: {})",
    ...
    if (!spellInfo)
    {
        LOG_ERROR("entities.player", "bot_ai::RefreshAura(): Invalid spellInfo for spell {}! Bot - %s (botclass: {}, entry: {})",
...
Unit* bot_ai::_getVehicleTarget(BotVehicleStrats /*strat*/) const
{
    ...
    Group * gr = !IAmFree() ? master->GetGroup() : nullptr;
    ...
    if (mmover->IsAlive())
    {
            ...
            (mytar && mmover->GetDistance(mytar) > followdist / 2 && !mytar->IsWithinLOSInMap(veh)))
        {
            //if (mytar)
            //{
            //    LOG_ERROR("scripts", "_getVehicleTarget %s's veh is too far from master - lost target (%.2f > %.2f)",
...
Unit* bot_ai::_getTarget(bool byspell, bool ranged, bool &reset) const
{
    ...
    //LOG_ERROR("entities.player", "bot_ai::getTarget(): bot: %s", me->GetName().c_str());
    ...
    if (gr && IsTank())
    {
        ...
        for (int8 i = TARGETICONCOUNT - 1; i >= 0; --i)
        {
            if (BotMgr::GetTankTargetIconFlags() & GroupIconsFlags[i])
            {
                if (ObjectGuid guid = gr->GetTargetIcons()[i])
                {
                    ...
                    {
                        //LOG_ERROR("entities.unit", "_getTarget: %s continues %s", me->GetName().c_str(), mytar->GetName().c_str());
                        return mytar;
                    }
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        ...
                        {
                            //LOG_ERROR("entities.unit", "_getTarget: %s found new tanking icon target %s", me->GetName().c_str(), unit->GetName().c_str());
                            Unit* tempTar = tankTar ? tankTar : unit;
                            tankTar = unit;
                            Unit* tVic = unit->GetVictim();
                            if (!tVic || (tVic != me && tVic->GetVictim() == unit && IsTank(tVic) && IsInBotParty(tVic)))
                            {
                                //LOG_ERROR("entities.unit", "_getTarget: %s skipped %s (%s)", me->GetName().c_str(), unit->GetName().c_str(), tVic->GetName().c_str());
                                tankTar = tempTar;
                                continue;
                            }
                        }
        ...
        if (tankTar)
        {
            //LOG_ERROR("entities.unit", "_getTarget: %s returning %s", me->GetName().c_str(), tankTar->GetName().c_str());
            return tankTar;
        }
    }
    if (gr)
                ...
                if (ObjectGuid guid = gr->GetTargetIcons()[i])
                {
                    ...
                    if (Unit* unit = ObjectAccessor::GetUnit(*me, guid))
                    {
                        if (unit->IsVisible() && unit->isTargetableForAttack(false) && me->IsValidAttackTarget(unit) &&
                            unit->IsInCombat() && (CanSeeEveryone() || (me->CanSeeOrDetect(unit) && unit->InSamePhase(me))))
                        {
                            //LOG_ERROR("entities.unit", "_getTarget: found dps icon target %s", unit->GetName().c_str());
                            return unit;
                        }
                    }
                }
...
*/
    ...
    {
        //LOG_ERROR("entities.player", "bot %s continues attack common target %s", me->GetName().c_str(), u->GetName().c_str());
        return u;//forced
    }
    //Follow if...
    uint8 followdist = IAmFree() ? BotMgr::GetBotFollowDistDefault() : master->GetBotMgr()->GetBotFollowDist();
    ...
    {
        //LOG_ERROR("entities.player", "bot %s cannot attack target %s, too far away or not in LoS", me->GetName().c_str(), mytar ? mytar->GetName().c_str() : "");
        return nullptr;
    }

    if (u && !IAmFree() && (master->IsInCombat() || u->IsInCombat())/* && !InDuel(u)*/ && !IsInBotParty(u) && (BotMgr::IsPvPEnabled() || !u->IsControlledByPlayer()))
    {
        //LOG_ERROR("entities.player", "bot %s starts attack master's target %s", me->GetName().c_str(), u->GetName().c_str());
        return u;
    }
    ...
    {
        //LOG_ERROR("entities.player", "bot %s continues attack its target %s", me->GetName().c_str(), mytar->GetName().c_str());
    ...
    if (!IAmFree())
    {
        if (!gr)
        {
            ...
            for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
            {
                ...
                {
                    //LOG_ERROR("entities.player", "bot %s hooked %s's victim %s", me->GetName().c_str(), bot->GetName().c_str(), u->GetName().c_str());
                    return u;
                }
            }
        }
        else
        {
            ...
            for (GroupReference const* ref = gr->GetFirstMember(); ref != nullptr; ref = ref->next())
            {
                ...
                if (u && pl != master &&
                    ...
                    CanBotAttack(u, byspell))
                {
                    //LOG_ERROR("entities.player", "bot %s hooked %s's victim %s", me->GetName().c_str(), pl->GetName().c_str(), u->GetName().c_str());
                    return u;
                }
    ...
    //if (t)
    //    LOG_ERROR("entities.player", "bot %s has found new target %s", me->GetName().c_str(), t->GetName().c_str());
...
bool bot_ai::CheckAttackTarget()
{
    ...
    if (!opponent)
    {
        //LOG_ERROR("entities.player", "bot_ai: CheckAttackTarget() - bot %s lost target", me->GetName().c_str());
        if (me->GetVictim() || me->IsInCombat()/* || !me->GetThreatManager().isThreatListEmpty()*/)
        {
            //LOG_ERROR("entities.player", "bot_ai: CheckAttackTarget() - bot %s Evades", me->GetName().c_str());
            if (me->GetVictim())
                me->AttackStop();
...
void bot_ai::CalculateAoeSpots(Unit const* unit, AoeSpotsVec& spots)
{
    ...
    //if (!doList.empty())
    //    LOG_ERROR("scripts", "CalculateAoeSpots {} aoes around %s", uint32(doList.size()), unit->GetName().c_str());
    ...
    for (std::list<WorldObject*>::const_iterator ci = doList.begin(); ci != doList.end(); ++ci)
    {
        ...
        {
            //LOG_ERROR("scripts", "CalculateAoeSpots found %s's aoe %s ({}) radius %.2f size %.2f",
            //    dObj->GetCaster()->GetName().c_str(), spellInfo->SpellName[0], spellInfo->Id, dObj->GetRadius(), dObj->GetObjectSize());

            float radius = dObj->GetRadius() + 0.5f;
            radius += (unit->GetVehicle() ? unit->GetVehicleBase()->GetCombatReach() : 1.5f) * 1.2f;
    ...
    if (unit->GetMapId() == 616 && unit->GetVehicle())
    {
        ...
        Acore::AllCreaturesOfEntryInRange check2(unit->GetVehicleBase(), 30592, 60.f); //Static Field
    ...
    {
        std::list<Creature*> cList;
        Acore::AllCreaturesOfEntryInRange check2(unit, 23920, 40.f); //Fire Bomb (Zul'Aman)
        ...
        for (std::list<Creature*>::const_iterator ci = cList.begin(); ci != cList.end(); ++ci)
        {
            float radius = spellInfo->Effects[0].CalcRadius() + 1.5f * 1.2f;
            spots.push_back(AoeSpotsVec::value_type(*(*ci), radius));
        }
    }
}
...
bool bot_ai::IsWithinAoERadius(Position const& pos) const
{
    if (!IAmFree())
    {
        ...
        if (!spots.empty())
        {
            ...
            float cr_diff = mover->GetCombatReach() - 1.5f;
...
void bot_ai::CalculateAttackPos(Unit* target, Position& pos, bool& force) const
{
    ...
    if (me->GetVehicle())
    {
        ...
        for (uint8 i = 0; i < 4; ++i)
        {
            if (me->GetVehicleBase()->CanFly())
            {
                ...
                target->GetNearPoint(me->GetVehicleBase(), tx, ty, tz, dist, 0.0f, Position::NormalizeOrientation(angle));
            ...
            if (isinaoe)
            {
                //LOG_ERROR("scripts", "CalculateAttackPos %s veh skipped aoe pos", me->GetName().c_str());
                if (me->GetVehicleBase()->CanFly())
    ...
    if (!aoespots.empty())
    {
        ...
        {
            //if (!aoespots.empty())
            //    LOG_ERROR("scripts", "CalculateAttackPos %s spot is still safe", me->GetName().c_str());
    ...
    if (!safespots.empty())
    {
        ...
        //LOG_ERROR("scripts", "CalculateAttackPos {} safe spots, chosen at dist %.2f", uint32(safespots.size()), mindist);
...
void bot_ai::GetInPosition(bool force, Unit* newtarget, Position* mypos)
{
    ...
    if (HasRole(BOT_ROLE_RANGED) || HasVehicleRoleOverride(BOT_ROLE_RANGED) || (!IAmFree() && !GetAoeSpots().empty()))
    {
        ...
        //LOG_ERROR("scripts", "GetInPosition %s to %s dist %.2f, to pos %.2f", me->GetName().c_str(),
...
//        return;
    ...
    //LOG_ERROR("entities.player", "AdjustTankingPosition() by %s", me->GetName().c_str());
    ...
    //LOG_ERROR("entities.player", "AdjustTankingPosition(): atts {}, behind {}", uint32(myattackers.size()), bCount);
...
void bot_ai::MoveBehind(Unit const* target) const
{
    ...
    {
        Position position;
        target->GetNearPoint(me, position.m_positionX, position.m_positionY, position.m_positionZ, me->GetCombatReach(), 0.0f, target->GetAbsoluteAngle(me) + M_PI);
...
void bot_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!newSpell)
    {
        ...
        if (npcBotData->disabled_spells.find(basespell) != npcBotData->disabled_spells.end())
        {
            ...
            //LOG_ERROR("entities.player", "bot_ai::InitSpellMap(): %s ({} -> {}) is disabled for %s!",
...
void bot_ai::_OnHealthUpdate() const
{
    ...
    if (myclass == BOT_CLASS_DRUID && GetBotStance() != BOT_STANCE_NONE)
        myclass = GetBotStance();
    //LOG_ERROR("entities.player", "_OnHealthUpdate(): updating bot %s", me->GetName().c_str());
    ...
    //LOG_ERROR("entities.player", "class base health: {}", m_basehp);
    ...
    //LOG_ERROR("entities.player", "bot's stats to health add: Stamina (%f), value: %f", stamValue, stamValue * 10.f);
    ...
    //LOG_ERROR("entities.player", "health to add after stam mod: %i", hp_add);
    uint32 m_totalhp = m_basehp + int32(hp_add); //m_totalhp = uint32(float(m_basehp + hp_add) * stammod);
    //LOG_ERROR("entities.player", "total base health: {}", m_totalhp);
    ...
    //LOG_ERROR("entities.player", "overall hp: {}", me->GetMaxHealth());
...
void bot_ai::_OnManaUpdate() const
{
    ...
    //LOG_ERROR("entities.player", "_OnManaUpdate(): updating bot %s", me->GetName().c_str());
    ...
    //LOG_ERROR("entities.player", "classinfo base mana = %f", m_basemana);
...
void bot_ai::OnSpellHit(Unit* caster, SpellInfo const* spell)
{
    ...
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
    {
        ...
        {
            //LOG_ERROR("entities.unit", "OnSpellHit: mount on %s", me->GetName().c_str());
            UnsummonAll();
            ...
            {
                //LOG_ERROR("entities.unit", "OnSpellHit: modding flight speed");
                ...
                {
                    //LOG_ERROR("entities.unit", "OnSpellHit: found aura");
...
void bot_ai::OnSpellHitTarget(Unit* /*target*/, SpellInfo const* spell)
{
    if (me->GetVehicle())
    {
        ...
        if (spellId == 56091 || spellId == 57090)
        {
            ...
            //LOG_ERROR("scripts", "OnBotSpellGo(): veh cp spell {} now cp {}", curInfo->Id, uint32(vehcomboPoints));
        }
        //Engulf in Flames, Life Burst, Flame Shield   moved to globalupdate
        if (spellId == 56092 || spellId == 57143 || spellId == 57108)
        {
            vehcomboPoints = 0;
            //LOG_ERROR("scripts", "OnSpellHitTarget(): veh cp waster {}", curInfo->Id);
        }
    }
}
//Update delay
...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
            ...
            switch (_botclass)
            {
                case BOT_CLASS_MAGE:
                {
                    ...
                    else if (option == 3) //refreshment table
                    {
                        ...
                        G3D::Matrix3 rot = G3D::Matrix3::fromEulerAnglesZYX(me->GetOrientation(), 0.f, 0.f);
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            //LOG_ERROR("entities.player", "OnGossipSelect(bot): added {} item(s) to list of %s (requester: %s)",
            ...
            else
            {
                ...
                switch (reason)
                {
                    case 1: //has owner
                    {
                        ...
                        if (auto pPlayer = ObjectAccessor::FindPlayerByLowGUID(_ownerGuid))
                            ostr << pPlayer->GetName();
                        else
        ...
        case GOSSIP_SENDER_DEBUG:
        {
            ...
            ostr << "Bot: " << me->GetName()
                ...
                << "). owner: ";
            if (auto pPlayer = ObjectAccessor::FindPlayerByLowGUID(_ownerGuid))
                ostr << pPlayer->GetName() << " (" << _ownerGuid << ')';
            else
                ostr << "none";
...
void bot_ai::OnOwnerDamagedBy(Unit* attacker)
{
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_ARCHMAGE:
            ...
            break;
        default:
            //LOG_ERROR("entities.player", "minion_ai: OnOwnerDamagedBy() - unknown bot class {}", uint8(_botclass));
            break;
    }
...
bool bot_ai::_canLootItemForPlayer(Player* player, Creature* creature, uint8 slot) const
{
    ...
    {
        //LOG_ERROR("scripts", "can't loot item {}, no item", slot);
        return false;
    }

    if (!qitem && item->is_blocked)
    {
        //LOG_ERROR("scripts", "can't loot item {} ({}), blocked", slot, item->itemid);
        return false;
    }

    if (!item->rollWinnerGUID.IsEmpty() && item->rollWinnerGUID != player->GetGUID())
    {
        //LOG_ERROR("scripts", "can't loot item {} ({}), roll won", slot, item->itemid);
        return false;
    }
    ...
    //LOG_ERROR("scripts", "can't loot item {} ({}), can't store", slot, item->itemid);
    return false;
}
bool bot_ai::_canLootCreatureForPlayer(Player* player, Creature* creature, uint32 lootQualityMask, uint32 lootThreshold) const
{
    ...
    for (std::vector<LootItem>::const_iterator i = creature->loot.items.begin(); i != creature->loot.items.end(); ++i)
    {
        ...
        {
            //LOG_ERROR("scripts", "item {} is blocked", i->itemid);
            continue;
        }

        if (!i->rollWinnerGUID.IsEmpty() && i->rollWinnerGUID != player->GetGUID())
        {
            //LOG_ERROR("scripts", "can't loot item {} ({}), roll won", slot, i->itemid);
            continue;
        }
        ...
        if (!itemProto)
        {
            //LOG_ERROR("scripts", "no item proto for itemId {}", i->itemid);
            return false;
        }

        if (itemProto->Quality >= lootThreshold)
        {
            //LOG_ERROR("scripts", "item {} group quality threshold mismatch", i->itemid);
            continue;
        }

        if (!((1 << itemProto->Quality) & lootQualityMask))
        {
            //LOG_ERROR("scripts", "item {} lootQualityMask mismatch", i->itemid);
            continue;
        }
...
bool bot_ai::_canLootCreature(Creature* creature) const
{
    ...
    //LOG_ERROR("scripts", "lootQualityMask {}, lootThreshold {}", lootQualityMask, lootThreshold);
    ...
    for (std::vector<LootItem>::const_iterator ci = loot->quest_items.begin(); ci != loot->quest_items.end(); ++ci)
    {
        ...
        if (!itemProto)
        {
            //LOG_ERROR("scripts", "no item proto for itemId {}", ci->itemid);
            return false;
        }
    ...
    if (!canLootQuality)
    {
        //LOG_ERROR("scripts", "can't loot by quality");
        return false;
    }
    ...
    if (!canLootPlayers)
    {
        //LOG_ERROR("scripts", "can't loot by canLootPlayers");
        return false;
    }
...
void bot_ai::_autoLootCreatureGold(Creature* creature) const
{
    ...
    if (loot->isLooted())
    {
        //LOG_ERROR("scripts", "creature gold is looted, releasing");
...
void bot_ai::_autoLootCreatureItems(Player* receiver, Creature* creature, uint32 lootQualityMask, uint32 lootThreshold) const
{
    ...
    {
        //LOG_ERROR("scripts", "creature items is looted, releasing");
        creature->AllLootRemovedFromCorpse();
        creature->RemoveFlag(UNIT_DYNAMIC_FLAGS, UNIT_DYNFLAG_LOOTABLE);
        creature->loot.clear();
    }
...
bool bot_ai::_equip(uint8 slot, Item* newItem, ObjectGuid::LowType receiver)
{
    ...
    if (newItem->GetState() == ITEM_REMOVED)
    {
        LOG_ERROR("entities.player",
            "minion_ai::_equip(): player %s (guidLow: {}) is trying to make bot %s (id: {}) equip item: %s (id: {}, guidLow: {}) which has state ITEM_REMOVED!",
    ...
    if (slot > BOT_SLOT_RANGED || einfo->ItemEntry[slot] != newItemId)
    {
        ...
        if (newItem->GetOwnerGUID() != master->GetGUID() || !master->HasItemCount(newItemId, 1))
        {
            ...
            LOG_ERROR("entities.player",
                "minion_ai::_equip(): player %s (guidLow: {}) is trying to make bot %s (id: {}) equip item: %s (id: {}, guidLow: {}) but either does not have this item or does not own it",
                master->GetName().c_str(), master->GetGUID().GetCounter(), me->GetName().c_str(), me->GetEntry(), proto->Name1.c_str(), proto->ItemId, newItem->GetGUID().GetCounter());
            return false;
        }
...
bool bot_ai::_resetEquipment(uint8 slot, ObjectGuid::LowType receiver)
{
    ...
    {
        LOG_ERROR("entities.player", "minion_ai::_resetEquipment(): player %s (guidLow: {}) failed to reset equipment for bot %s (id: {}) in slot {}",
        ...
        switch (enchant_display_type)
        {
            ...
            case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
                if (enchant_spell_id)
                {
                    ...
                    //}
                    me->CastSpell(me, enchant_spell_id, false);
                }
                break;
            case ITEM_ENCHANTMENT_TYPE_RESISTANCE:
                ...
                _stats[slot][BOT_STAT_MOD_RESISTANCE_START];
                break;
...
void bot_ai::ApplyItemSetBonuses(Item* item, bool apply)
{
    if (item) //(un)equip, NOT from loops
    {
        ...
        for (uint8 i = 0; i != MAX_ITEM_SET_SPELLS; ++i)
        {
            ...
            //LOG_ERROR("entities.player", "ApplyItemSetBonusesB: %s's %s, %s ({}), %s ({}), icount {}",
    ...
    for (std::list<uint32>::const_iterator itr = itemSets.begin(); itr != itemSets.end(); ++itr)
    {
        ...
        for (uint8 j = 0; j != MAX_ITEM_SET_SPELLS; ++j)
        {
            ...
            //LOG_ERROR("entities.player", "ApplyItemSetBonusesB (all): %s's %s, %s ({}), %s ({}), c {}, req {}",
...
float bot_ai::_getItemGearScore(Item const* item, uint8 forslot) const
{
    ...
    //LOG_ERROR("scripts", "_getItemGearScore for {} - %s", item->GetEntry(), proto->Name1.c_str());

    ItemStatBonus istats = {};
    //for (uint8 i = 0; i != MAX_BOT_ITEM_MOD; ++i)
    //    LOG_ERROR("scripts", "_getItemGearScore at {} %i", uint32(i), istats[i]);
        ...
        for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            switch (enchant_display_type)
            {
                ...
                case ITEM_ENCHANTMENT_TYPE_RESISTANCE:
                    ...
                    istats[BOT_STAT_MOD_RESISTANCE_START];
                    break;
                case ITEM_ENCHANTMENT_TYPE_STAT:
                {
                    if (!enchant_amount)
    ...
    //for (uint8 i = 0; i != MAX_BOT_ITEM_MOD; ++i)
    //    LOG_ERROR("scripts", "_getItemGearScore total {} %i", uint32(i), istats[i]);
    ...
    //LOG_ERROR("scripts", "_getItemGearScore total score %.3f", itemScore);
    return itemScore;
}
...
void bot_ai::_saveStats()
{
    ...
    stats.maxpower = me->GetMaxPower(_botclass == BOT_CLASS_DRUID ? POWER_MANA : (Powers)me->getPowerType());
...
float bot_ai::GetAverageItemLevel() const
{
    ...
    for (uint8 i = BOT_SLOT_MAINHAND; i != BOT_INVENTORY_SIZE; ++i)
    {
        if (_equips[i] && !(/*i == BOT_SLOT_TABARD || */i == BOT_SLOT_OFFHAND || i == BOT_SLOT_RANGED || i == BOT_SLOT_BODY))
        {
            ...
            {
                ++count;
                sum += itemProto->GetItemLevelIncludingQuality(itemProto->ItemLevel);
            }
        }
    }
...
void bot_ai::ApplyRacials()
{
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            ...
            break;
        default:
            //LOG_ERROR("entities.player", "bot_ai::ApplyRacePassives(): unknown race {} for bot %s ({})", uint32(me->getRace()), me->GetName().c_str(), me->GetEntry());
            return;
    }
}
...
void bot_ai::InitEquips()
{
    ...
    else
    {
        do
        {
            ...
            ASSERT(found);
            //ItemTemplate const* proto = item->GetTemplate();
            //LOG_ERROR("entities.player", "minion_ai::InitEquips(): bot %s (id: {}): found item: for slot {}: %s (id: {}, guidLow: {})",
...
void bot_ai::_AddItemLink(Player const* forPlayer, Item const* item, std::ostringstream &str, bool addIcon) const
{
    ...
    //LOG_ERROR("entities.player", "bot_ai::_AddItemLink(): %s", str.str().c_str());
}
//Unused
...
void bot_ai::_LocalizeItem(Player const* forPlayer, std::string &itemName, std::string &suffix, Item const* item) const
{
    ...
    //LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): randomPropId = %i", randomPropId);
    if (!randomPropId)
        return;
    ...
    if (randomPropId > 0)
    {
        if (ItemRandomPropertiesEntry const* item_rand = sItemRandomPropertiesStore.LookupEntry(randomPropId))
        {
            ...
            //    LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): rand prop suffix for loc {} = %s", i, suffs[i]);
            suffix = item_rand->Name[loc];
        }
    }
    else
    {
        ...
        {
            //std::array<char const*, 16> const& suffs = item_rand->nameSuffix;
            //if (suffs)
            {
                //for (uint8 i = 0; i != MAX_LOCALES; ++i)
                //    LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): rand suff suffix for loc {} = %s", i, suffs[i]);
                suffix = item_rand->Name[loc];
            }
        }
    }
...
void bot_ai::JustDied(Unit*)
{
    ...
    if (IsTempBot())
    {
        //LOG_ERROR("entities.player", "Unsummoning temp bot %s (guidLow: {}), owner: %s (guidLow: {})...",
...
void bot_ai::OnBotSpellGo(Spell const* spell, bool ok)
{
    ...
    {
        if (DEBUG_BOT_ORDERS)
            LOG_ERROR("entities.player", "doCast(): ordered spell {} by %s was %s!",
...
void bot_ai::OnBotOwnerSpellGo(Spell const* spell, bool ok)
{
    ...
    //LOG_ERROR("entities.player", "OnBotOwnerSpellGo(): {} by %s", spellInfo->Id, master->GetName().c_str());
...
void bot_ai::OnBotSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        //LOG_ERROR("entities.player", "OnBotSpellInterrupted(): Adding cooldown ({}, new: {}) to spell %s (id: {}, schoolmask: {}), reqSchoolMask = {}",
...
void bot_ai::CastBotItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item, ItemTemplate const* proto)
{
    ...
    if (procVictim & PROC_FLAG_TAKEN_DAMAGE)
    {
        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (!spellInfo)
            {
                //LOG_ERROR("entities.player.items", "WORLD: unknown Item spellid %i", spellData.SpellId);
                continue;
            }
    ...
    for (uint8 e_slot = 0; e_slot != MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                //LOG_ERROR("entities.player.items", "Player::CastItemCombatSpell(GUID: {}, name: %s, enchant: %i): unknown spell %i is casted, ignoring...",
            ...
            {
                //CastSpellExtraArgs args(item);
                me->CastSpell(spellInfo->IsPositive() ? me : target, spellInfo->Id, false);
            }
        }
    }
}
...
bool bot_ai::AddOrder(BotOrder const* order)
{
    ...
    {
        LOG_ERROR("scripts", "bot_ai::AddOrder: orders limit reached for %s ({})!", me->GetName().c_str(), uint32(_orders.size()));
        return false;
    }
...
void bot_ai::CancelOrder(BotOrder const* order)
{
    if (_orders.empty())
    {
        LOG_ERROR("scripts", "bot_ai::CancelOrder: %s orders are empty while trying to remove order type {}!",
            me->GetName().c_str(), uint32(order->_type));
        return;
    }
    if (_orders.front()->_type != order->_type || _orders.front()->params.whole != order->params.whole)
    {
        LOG_ERROR("scripts", "bot_ai::CancelOrder: %s front order (type {}) is different from cur order (type {})!",
...
void bot_ai::CompleteOrder(BotOrder const* order)
{
    if (_orders.empty())
    {
        LOG_ERROR("scripts", "bot_ai::CompleteOrder: %s orders are empty while trying to remove order type {}!",
            me->GetName().c_str(), uint32(order->_type));
        return;
    }
    if (_orders.front()->_type != order->_type || _orders.front()->params.whole != order->params.whole)
    {
        LOG_ERROR("scripts", "bot_ai::CompleteOrder: %s front order (type {}) is different from cur order (type {})!",
            me->GetName().c_str(), uint32(_orders.front()->_type), uint32(order->_type));
        return;
    }
...
void bot_ai::_ProcessOrders()
{
    ...
    switch (order->_type)
    {
        case BOT_ORDER_SPELLCAST:
        {
            ...
            {
                LOG_ERROR("scripts", "bot_ai:_ProcessOrders: invalid spellCastParams.targetGuid ({})!", order->params.spellCastParams.targetGuid);
                CancelOrder(order);
                return;
            }
            ...
            {
                LOG_ERROR("scripts", "bot_ai:_ProcessOrders: target ({}) not found!", order->params.spellCastParams.targetGuid);
                CancelOrder(order);
                return;
            }
...
void bot_ai::DoSkytalonVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        LOG_ERROR("scripts", "DoSkytalonVehicleStrats no spell for role mask {} cp {}, power {}, target %s",
            GetBotRoles(), uint32(vehcomboPoints), drakePower, target->GetName().c_str());
        return;
    }

    //LOG_ERROR("scripts", "DoSkytalonVehicleStrats {} on %s", drakespell, target->GetName().c_str());
...
void bot_ai::DoRubyDrakeVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        LOG_ERROR("scripts", "DoRubyDrakeVehicleStrats no spell for target %s", target->GetName().c_str());
        return;
    }
...
void bot_ai::DoEmeraldDrakeVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        LOG_ERROR("scripts", "DoEmeraldDrakeVehicleStrats no spell for target %s", target->GetName().c_str());
        return;
    }
...
void bot_ai::DoAmberDrakeVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        LOG_ERROR("scripts", "DoAmberDrakeVehicleStrats no spell for target %s", target->GetName().c_str());
        return;
    }
...
void bot_ai::DoArgentMountVehicleStrats(uint32 diff)
{
    ...
    if (!mountspell)
    {
        LOG_ERROR("scripts", "DoArgentMountVehicleStrats no spell for target %s", target->GetName().c_str());
        return;
    }
...
void bot_ai::DoVehicleStrats(BotVehicleStrats strat, uint32 diff)
{
    ...
    {
        //LOG_ERROR("scripts", "DoVehicleStrats doing strat {}", uint32(strat));
        curVehStrat = strat;
    }

    //if (!master->GetVehicle() || me->GetVehicle()->GetCreatureEntry() != master->GetVehicle()->GetCreatureEntry())
    //{
    //    LOG_ERROR("scripts", "DoVehicleStrats cannot do strats: master not on vehicle or on different one");
    //    return;
    //}
    ...
    switch (me->GetVehicleBase()->GetEntry())
    {
        ...
        */
        default:
            ...
            if (curVehStrat != strat)
                LOG_ERROR("scripts", "bot_ai DoVehicleActions: %s has to use generic strat for vehicle creature %s ({})",
...
bool bot_ai::GlobalUpdate(uint32 diff)
{
    ...
    if (_updateTimerEx1 <= diff && !IAmFree())
    {
        ...
        */
        ...
        {
            me->SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS, 0.5f * me->GetObjectScale());
            me->SetFloatValue(UNIT_FIELD_COMBATREACH,  1.5f * me->GetObjectScale());
        }
        //end DEBUG
        ...
        {
            //LOG_ERROR("spells", "bot_ai:UpdateEx by %s, found free master, my skills: {}:", me->GetName().c_str(), uint32(_roleMask & BOT_ROLE_MASK_GATHERING));
            ...
            //LOG_ERROR("spells", "bot_ai:UpdateEx: list is %s", woList.empty() ? "empty" : "not empty");
            if (!woList.empty())
            {
                WorldObject* wo = Acore::Containers::SelectRandomContainerElement(woList);
                //LOG_ERROR("spells", "bot_ai:UpdateEx: processing %s", wo->GetName().c_str());
                ...
                else
                {
                    ...
                    {
                        Position pos;
                        wo->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, CONTACT_DISTANCE, 0.0f, wo->GetAbsoluteAngle(me));
        ...
        if (me->IsInWorld() && HasRole(BOT_ROLE_AUTOLOOT) && HasRole(BOT_ROLE_MASK_LOOTING) &&
            ...
            !HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
        {
            //LOG_ERROR("scripts", "bot_ai Looting by %s, my mask: {}:", me->GetName().c_str(), uint32(_roleMask & BOT_ROLE_MASK_LOOTING));
            ...
            for (std::list<Unit*>::iterator itr = crList.begin(); itr != crList.end();)
            {
                ...
                {
                    //LOG_ERROR("scripts", "bot_ai Looting: cannot loot %s ({})", (*itr)->GetName().c_str(), (*itr)->GetGUIDLow());
                    crList.erase(itr++);
                    continue;
                }
                itr++;
            ...
            if (!crList.empty())
            {
                ...
                /*//LOG_ERROR("scripts", "bot_ai Looting: processing %s", un->GetName().c_str());
                if (me->GetDistance(un) < INTERACTION_DISTANCE + 2.f)
                {
                    //LOG_ERROR("scripts", "bot_ai Looting: looting %s", un->GetName().c_str());
                    _autoLootCreature(un->ToCreature());
                    _updateTimerEx1 = urand(500, 1000);
                }
                else
                {
                    //LOG_ERROR("scripts", "bot_ai Looting: moving towards %s", un->GetName().c_str());
                    Position pos;
    ...
    if (checkAurasTimer <= lastdiff)
    {
        ...
        {
            //LOG_ERROR("entities.player", "CheckAuras(): adding %s to group", me->GetName().c_str());
        ...
        {
            //LOG_ERROR("scripts", "GetInPos prepare by %s", me->GetName().c_str());
            if (!IAmFree() && master->GetBotMgr()->GetBotAttackRangeMode() == BOT_ATTACK_RANGE_EXACT &&
            ...
            {
                //LOG_ERROR("scripts", "%s calculates attack pos to attack %s", me->GetName().c_str(), victim->GetName().c_str());
                bool force = false;
                CalculateAttackPos(victim, attackpos, force);
                if (force || mover->GetExactDist2d(&attackpos) > 4.f)
                {
                    //LOG_ERROR("scripts", "%s moving to x %.2f y %.2f z %.2f to attack %s",
...
void bot_ai::Evade()
{
    ...
    if (mapid != me->GetMapId() || _evadeCount >= 3 || me->GetDistance(pos) > 1000.f)
    {
        ...
        if (!teleHomeEvent)
        {
            ...
            Events.AddEvent(teleHomeEvent, Events.CalculateTime(5));
...
void bot_ai::OnBotEnterVehicle(Vehicle * vehicle)
{
    if (VehicleSeatEntry const* seat = vehicle->GetSeatForPassenger(me))
    {
        ...
        if (Unit* oVeh = master->GetVehicleBase())
        {
            ...
            //}
            //speed
            if (vehicle->GetBase()->GetTypeId() == TYPEID_UNIT && vehicle->GetBase()->ToCreature()->CanFly())
...
void bot_ai::OnBotExitVehicle(Vehicle * vehicle)
{
    if (VehicleSeatEntry const* seat = vehicle->GetSeatForPassenger(me))
    {
        if (seat->m_flags & VEHICLE_SEAT_FLAG_CAN_CONTROL)
...
void bot_ai::AfterBotOwnerEnterVehicle()
{
        ...
        master->GetVehicleBase()->IsControlledByPlayer())
    {
        ...
        if (seat && seat->CanEnterOrExit())
        {
            ...
            if (!creEntry)
            {
                LOG_ERROR("scripts", "OnBotOwnerEnterVehicle: no vehicle selected for bot master veh %s!",
    ...
    //LOG_ERROR("scripts", "OnBotOwnerEnterVehicle: master not in vehicle or no veh found for bot %s!", me->GetName().c_str());
}
...
void bot_ai::OnBotOwnerExitVehicle(Vehicle const* /*vehicle*/)
{
    ...
    */
    //LOG_ERROR("scripts", "OnBotOwnerExitVehicle: no vehicle or no seat for bot %s!", me->GetName().c_str());
}
...
Unit* bot_ai::SpawnVehicle(uint32 creEntry, uint32 vehEntry)
{
    ...
    TempSummon* vc = map->SummonCreature(creEntry, Position(x,y,z), nullptr, 0, nullptr, 0, vehEntry);
    //TempSummon* vc = new TempSummon(nullptr, me, false);
    ASSERT(vc->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, master->GetPhaseMask(), creEntry, vehEntry, x, y, z, o, nullptr));
...
void bot_ai::ChooseVehicleForEncounter(Player const* owner, uint32 &creEntry, uint32 &vehEntry) const
{
    ...
    switch (mVeh->GetBase()->GetEntry())
    {
        ...
        default:
            ...
            LOG_ERROR("scripts", "ChooseVehicleForEncounter: unhandled master vehicle creature %s ({})",
...
uint8 bot_ai::GetPlayerClass() const
{
    if (_botclass >= BOT_CLASS_EX_START)
    {
        switch (_botclass)
        {
            ...
            default:
                LOG_ERROR("entities.unit", "GetPlayerClass: %s has unknown Ex bot class {}!", me->GetName().c_str(), _botclass);
                return BOT_CLASS_PALADIN;
        }
    }
    ...
    if (_botclass >= BOT_CLASS_EX_START)
    {
        switch (_botclass)
        {
            ...
            case BOT_CLASS_DARK_RANGER:
                return RACE_BLOODELF;
            default:
                LOG_ERROR("entities.unit", "GetPlayerRace: %s has unknown Ex bot class {}!", me->GetName().c_str(), _botclass);
                return RACE_HUMAN;
        }
    }
...
void bot_ai::InitBotCustomSpells()
{
    ...
    if (SPELLS_DEFINED)
    {
        //LOG_ERROR("entities.player", "Bot custom spells initialization... fail...");
        return;
    }
    //LOG_ERROR("entities.player", "Bot custom spells initialization... success...");
    SPELLS_DEFINED = true;
    ...
    trigInfo->AttributesEx |= (SPELL_ATTR1_FAILURE_BREAKS_STEALTH);
    ...
    sinfo->AttributesEx |= (SPELL_ATTR1_FAILURE_BREAKS_STEALTH | SPELL_ATTR1_NO_THREAT);
    ...
    //sinfo->Attributes |= SPELL_ATTR0_HIDE_IN_COMBAT_LOG | SPELL_ATTR0_DO_NOT_DISPLAY | SPELL_ATTR0_DO_NOT_SHEATH;
    ...
    sinfo->Stances = 0;
    sinfo->Speed = 0.f;
    sinfo->Attributes |= SPELL_ATTR0_CASTABLE_WHILE_DEAD | SPELL_ATTR0_NO_IMMUNITIES;
    ...
    sinfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT | SPELL_ATTR3_NO_DONE_BONUS;
    ...
    sinfo->Attributes |= SPELL_ATTR0_IS_ABILITY | SPELL_ATTR0_DO_NOT_SHEATH | SPELL_ATTR0_IS_REPLENISHMENT | SPELL_ATTR0_HIDE_IN_COMBAT_LOG;
    ...
    */
    ...
    //sinfo->ExplicitTargetMask = TARGET_FLAG_UNIT;
    //sinfo->MaxAffectedTargets = 1000;
    sinfo->Attributes |= SPELL_ATTR0_DO_NOT_SHEATH | SPELL_ATTR0_IS_ABILITY | SPELL_ATTR0_CASTABLE_WHILE_MOUNTED;
    sinfo->Attributes &= ~(SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION);
    sinfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT;
    ...
    sinfo->AttributesEx2 |= SPELL_ATTR2_IGNORE_LINE_OF_SIGHT | SPELL_ATTR2_UNK22;
    sinfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT;
    ...
    */
    ...
    sinfo->AttributesEx |= SPELL_ATTR1_NO_REFLECTION | SPELL_ATTR1_CANT_BE_REDIRECTED | SPELL_ATTR1_NO_THREAT;
    sinfo->AttributesEx2 |= SPELL_ATTR2_CANT_CRIT;
    sinfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT | SPELL_ATTR3_DISABLE_PROC | SPELL_ATTR3_CAN_PROC_WITH_TRIGGERED | SPELL_ATTR3_NO_DONE_BONUS;
    ...
    sinfo->ExplicitTargetMask = TARGET_FLAG_UNIT;
    sinfo->Attributes &= ~(SPELL_ATTR0_NOT_SHAPESHIFTED | SPELL_ATTR0_HEARTBEAT_RESIST_CHECK);
    sinfo->AttributesEx |= SPELL_ATTR1_CANT_BE_REDIRECTED | SPELL_ATTR1_NO_REFLECTION;
    sinfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT;
    ...
    sinfo->ExplicitTargetMask = TARGET_FLAG_UNIT;
    sinfo->Attributes &= ~(SPELL_ATTR0_NOT_SHAPESHIFTED);
    sinfo->AttributesEx |= SPELL_ATTR1_CANT_BE_REDIRECTED | SPELL_ATTR1_NO_REFLECTION | SPELL_ATTR1_UNAFFECTED_BY_SCHOOL_IMMUNE;
    sinfo->AttributesEx2 |= SPELL_ATTR2_CANT_CRIT;
    sinfo->AttributesEx3 |= SPELL_ATTR3_ALWAYS_HIT;
    ...
    sinfo->Attributes |= SPELL_ATTR0_IMPOSSIBLE_DODGE_PARRY_BLOCK;
    sinfo->AttributesEx |= SPELL_ATTR1_CANT_BE_REDIRECTED | SPELL_ATTR1_NO_REFLECTION;
    sinfo->AttributesEx3 |= SPELL_ATTR3_NO_INITIAL_AGGRO | SPELL_ATTR3_ALWAYS_HIT;
    sinfo->AttributesEx4 |= SPELL_ATTR4_IGNORE_RESISTANCES;
    ...
    sinfo->AuraInterruptFlags = 0x0;
    sinfo->ExplicitTargetMask = TARGET_FLAG_UNIT | TARGET_FLAG_DEST_LOCATION;
    sinfo->AttributesEx |= SPELL_ATTR1_NO_REFLECTION | SPELL_ATTR1_CANT_BE_REDIRECTED;
