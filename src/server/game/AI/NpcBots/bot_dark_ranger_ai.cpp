...
public:
    ...
    struct dark_ranger_botAI : public bot_ai
    {
        ...
        void KilledUnit(Unit* u) override
        {
            ...
            if (u->GetGUID().GetRawValue() == _blackArrowKillGUID)
            {
        ...
        void OnBotDamageDealt(Unit* victim, uint32 damage, CleanDamage const* /*cleanDamage*/, DamageEffectType /*damagetype*/, SpellInfo const* spellInfo) override
        {
            ...
            if (damage && me->IsAlive() && victim->GetTypeId() == TYPEID_UNIT && damage >= victim->GetHealth() &&
                ...
                _blackArrowKillGUID = victim->GetGUID().GetRawValue();
        }
        ...
        void SummonBotPet(Unit* from)
        {
            ...
            {
                //LOG_ERROR("entities.player", "bot_dranger_ai::SummonBotPet(): max minions");
                Unit* u = nullptr;
                ...
                //if (u)
                //    LOG_ERROR("entities.player", "bot_dranger_ai::SummonBotPet(): found minion to erase(1)");
                //try 3: last resort
                if (!u)
                u = *(_minions.begin());
                //if (u)
                //    LOG_ERROR("entities.player", "bot_dranger_ai::SummonBotPet(): found minion to erase(2)");
        ...
        {
            //LOG_ERROR("entities.unit", "SummonedCreatureDies: %s's %s", me->GetName().c_str(), summon->GetName().c_str());
            //if (summon == botPet)
            //    botPet = nullptr;
        }
