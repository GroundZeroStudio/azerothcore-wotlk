...
public:
    ...
    struct rogue_botAI : public bot_ai
    {
        ...
        void Attack(uint32 diff)
        {
            ...
            {
                //LOG_ERROR("entities.player", "rogue_bot: cannot attack target...");
                return;
            }
            ...
            if (dist > 5)
            {
                ...
                //    LOG_ERROR("entities.player", "rogue_bot: cannot attack target (dist)...");
                return;
            }
            ...
            {
                //LOG_ERROR("entities.player", "bot_rogue: delaying attacks on gouged or blinded target...");
                return;
            }
            ...
            if (comboPoints > 0)
            {
                ...
                if (GetSpell(KIDNEY_SHOT_1) && !stealthed && stunDivider < DIMINISHING_LEVEL_4 &&
                    ...
                    !opponent->IsImmunedToSpellEffect(sSpellMgr->GetSpellInfo(KIDNEY_SHOT_1), EFFECT_0) &&
            ...
            if (spellInfo->NeedsComboPoints() && comboPoints)
            {
                if (lvl >= 10)
                {
                    if (irand(1,100) <= 20 * comboPoints)
                    {
                        ...
                        //LOG_ERROR("entities.player", "rogue_bot CP SPEND1: RS proc!");
                    }
                }
            }
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spell) override
        {
            ...
            {
                ++comboPoints;
                //LOG_ERROR("entities.player", "rogue_bot CP GEN2: %s adds 1, now {}", spell->SpellName[0], uint32(comboPoints));
                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //LOG_ERROR("entities.player", "rogue_bot CP NOR2: now {}", uint32(comboPoints));
                }
            }
                ...
                baseId == AMBUSH_1 || baseId == GARROTE_1 || baseId == CHEAP_SHOT_1/* || baseId == SHIV_1*/)
            {
                ...
                //LOG_ERROR("entities.player", "rogue_bot CP GEN1: %s adds {}, now {}",
                ...
                {
                    ++comboPoints;
                    //LOG_ERROR("entities.player", "rogue_bot CP GEN1: glyphSS proc, now {}", uint32(comboPoints));
                }

                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //LOG_ERROR("entities.player", "rogue_bot CP NOR1: now {}", uint32(comboPoints));
                }
            }
            ...
            else if (spell->NeedsComboPoints() && comboPoints)
            {
                ...
                //LOG_ERROR("entities.player", "rogue_bot CP SPEND1: {} to 0", tempCP);
                ...
                if (lvl >= 15)
                {
                    if (urand(1,100) <= 60)
                    {
                        ...
                        //LOG_ERROR("entities.player", "rogue_bot CP SPEND1: RU proc!");
                    }
                }
            }
            ...
            {
                //LOG_ERROR("entities.player", "rogue_bot Preparation hit!");
        ...
        void ApplyClassPassives() const override
        {
            ...
            //RefreshAura(VIGOR, level >= 20 ? 1 : 0); 会让盗贼能量变10
            ...
            //RefreshAura(VIGOR_GLADIATOR, level >= 70 ? 1 : 0); 会让盗贼能量变20
