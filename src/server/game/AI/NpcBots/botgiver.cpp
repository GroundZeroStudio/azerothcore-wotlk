...
public:
            ...
            switch (sender)
            {
                ...
                case HIRE_ENTRY:
                {
                    ...
                    if (bot->IsInCombat() || !bot->IsAlive() || bot_ai::CCed(bot)
                        || ai->IsDuringTeleport() || bot->HasUnitState(UNIT_STATE_CASTING) || ai->GetBotOwnerGuid() || bot->HasAura(BERSERK))
                    {
                        //LOG_ERROR("entities.unit", "HIRE_NBOT_ENTRY: bot {} (%s) is unavailable all of the sudden!", entry);
    ...
    {
        if (bot_giver_AI* ai = (bot_giver_AI*)creature->GetAI())
            return ai->OnGossipHello(player);
        return true;
    }
