...
void BotMgr::LoadConfig(bool reload)
{
    ...
    _maxNpcBots             = sConfigMgr->GetIntDefault("NpcBot.MaxBots", 4);
    ...
    _healTargetIconFlags    = sConfigMgr->GetIntDefault("NpcBot.HealTargetIconsMask", 1);
    _tankingTargetIconFlags = sConfigMgr->GetIntDefault("NpcBot.TankTargetIconMask", 2);
    _dpsTargetIconFlags     = sConfigMgr->GetIntDefault("NpcBot.DPSTargetIconMask", 128);
    ...
    _enableNpcBotsRaids     = sConfigMgr->GetBoolDefault("NpcBot.Enable.Raid", true);
    _enableNpcBotsBGs       = sConfigMgr->GetBoolDefault("NpcBot.Enable.BG", true);
    _enableNpcBotsArenas    = sConfigMgr->GetBoolDefault("NpcBot.Enable.Arena", true);
    ...
    _npcBotsCost            = sConfigMgr->GetIntDefault("NpcBot.Cost", 0);
    ...
    _npcBotOwnerExpireTime  = sConfigMgr->GetIntDefault("NpcBot.OwnershipExpireTime", 43000);
    ...
    _showHelm               = sConfigMgr->GetBoolDefault("NpcBot.EquipmentDisplay.ShowHelm", true);
...
void BotMgr::CleanupsBeforeBotDelete(ObjectGuid guid, uint8 /*removetype*/)
{
    ...
    if (!bot->IsInWorld())
    {
        ...
        //if (!bot->IsInWorld())
        //{
        //    LOG_ERROR("entities.player", "BotMgr::CleanupsBeforeBotDelete(): Failed to abort %s's teleport! Still out of world!", bot->GetName().c_str());
        //    ASSERT(false);
        //}
    }
...
bool BotMgr::RemoveBotFromGroup(Creature* bot)
{
    ...
    //    LOG_ERROR("entities.player", "RemoveBotFromGroup(): bot %s removed from group", bot->GetName().c_str());
    //else
    //    LOG_ERROR("entities.player", "RemoveBotFromGroup(): RemoveMember() returned FALSE on bot %s", bot->GetName().c_str());
...
int32 BotMgr::GetHPSTaken(Unit * unit) const
{
    if (!HaveBot())
        return 0;
    ...
    for (std::list<Unit*>::const_iterator itr = unitList.begin(); itr != unitList.end(); ++itr)
    {
        ...
        for (uint8 i = CURRENT_FIRST_NON_MELEE_SPELL; i != CURRENT_AUTOREPEAT_SPELL; ++i)
        {
            ...
            for (uint8 j = 0; j != MAX_SPELL_EFFECTS; ++j)
            {
                ...
                //LOG_ERROR("entities.player", "BotMgr:pendingHeals: found %s's %s on %s in %u (%i, total %i)",
    ...
    //if (amount != 0)
    //    LOG_ERROR("entities.player", "BotMgr:GetHPSTaken(): %s got %i)", unit->GetName().c_str(), amount);
...
void BotMgr::OnBotEnterVehicle(Creature const* passenger, Vehicle * vehicle)
{
    passenger->GetBotAI()->OnBotEnterVehicle(vehicle);
}

void BotMgr::OnBotExitVehicle(Creature const* passenger, Vehicle * vehicle)
{
