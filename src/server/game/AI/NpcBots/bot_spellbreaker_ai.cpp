...
public:
        ...
        void DamageDealt(Unit* victim, uint32& damage, DamageEffectType damageType) override
        {
            ...
            if (damage && victim != me && damageType == DIRECT_DAMAGE && victim->getPowerType() == POWER_MANA)
            {
                if (uint32 burned = std::min<uint32>(victim->GetPower(POWER_MANA), damage + me->getLevel() * 2))
                {
                    ...
                    me->CastSpell(victim, FEEDBACK_EFFECT, true);
                }
            }
    ...
    private:
        ...
        void ProcessSpellsteal(Unit* target)
        {
            ...
            //LOG_ERROR("entities.unit", "ProcessSpellsteal: on %s, fr={}", target->GetName().c_str(), uint32(isFriend));
            ...
            for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
            {
                ...
                if (!chance)
                    continue;
                //LOG_ERROR("entities.unit", "%s", aura->GetSpellInfo()->SpellName[0]);
                ...
                if (charges > 0)
                    steal_list.emplace_back(aura, charges);
            }

            if (steal_list.empty())
                return;

            //LOG_ERROR("entities.unit", "failcount...");

            size_t remaining = steal_list.size();
            uint32 failCount = 0;
            DispelChargesList success_list;
            success_list.resize(max_dispelled);
            WorldPacket dataFail(SMSG_DISPEL_FAILED, 8+8+4+4+max_dispelled*4);
            // dispel N = damage buffs (or while exist buffs for dispel)
            for (int32 count = 0; count < max_dispelled && !steal_list.empty();)
			{
				...
				else
				{
					...
					{
						success_list.push_back(std::make_pair(itr->first, itr->second));
						--itr->second;
						if (itr->second <= 0)
							steal_list.erase(itr);
					}
            ...
            //LOG_ERROR("entities.unit", "logs and auras");
        ...
        void TransferAura(uint32 spellId, ObjectGuid casterGUID, Unit* target, Unit* newTarget)
        {
            ...
            for (Unit::AuraMap::iterator iter = range.first; iter != range.second;)
            {
                ...
                if (aura->GetCasterGUID() == casterGUID)
                {
                    ...
                    int32 baseAmounts[MAX_SPELL_EFFECTS];
                    uint8 effMask = 0;
                    ...
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                    {
                        if (aura->GetEffect(i))
                        {
                            baseAmounts[i] = aura->GetEffect(i)->GetBaseAmount();
                            amounts[i] = aura->GetEffect(i)->GetAmount();
                            effMask |= (1<<i);
                    ...
                    else
                    {
                        ...
                        if (Aura* newAura = Aura::TryRefreshStackOrCreate(aura->GetSpellInfo(), effMask, newTarget, caster))
                        {
                            ...
                            newAura->SetLoadedState(aura->GetMaxDuration(), dur, aura->GetCharges(), aura->GetStackAmount(), recalculateMask, amounts);
