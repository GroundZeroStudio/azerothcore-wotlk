...
public:
    ...
    public:
        ...
        void UpdateAI(uint32 diff) override
        {
                ...
                me->GetPower(POWER_MANA) >= INFERNAL_COST && Rand() < 60)
            {
                ...
                if (target)
                    ...
                    me->GetNearPoint(me, _infernoPos.m_positionX, _infernoPos.m_positionY, _infernoPos.m_positionZ, 5.f, 0.f, 0.f);

                Position pos = _infernoPos.GetPosition();
                me->CastSpell(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), GetSpell(INFERNO_1), false);
                return;
            }
        ...
        void OnClassSpellGo(SpellInfo const* spellInfo) override
        {
            ...
            {
                Position pos = _infernoPos.GetPosition();
                me->CastSpell(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), SPELL_INFERNO_METEOR_VISUAL, true);
        ...
        void OnBotDamageDealt(Unit* victim, uint32 damage, CleanDamage const* /*cleanDamage*/, DamageEffectType /*damagetype*/, SpellInfo const* spellInfo) override
        {
            ...
            if (damage && victim != me && spellInfo && spellInfo->GetFirstRankSpell()->Id == CARRION_SWARM_1)
            {
                ...
                //LOG_ERROR("entities.unit", "OnBotDamageDealt(drl): %s on %s base val %i (%s),",
                //    me->GetName().c_str(), victim->GetName().c_str(), int32(damage), spellInfo->SpellName[0]);
                //CastSpellExtraArgs args(true);
                //args.AddSpellBP0(basepoints0);
                me->CastSpell(me, SPELL_TRIGGERED_HEAL, true);
            }
        }
