...
public:
    ...
    struct shaman_botAI : public bot_ai
    {
        ...
        void EnterCombat(Unit* u) override { TotemsCheckTimer = 0; bot_ai::EnterCombat(u); }
        ...
        void CheckTotems(uint32 diff)
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                    ...
                    me->GetDistance(_totems[i].second._pos) > _totems[i].second._effradius)
                {
                    ...
                    if (!to)
                    {
                        LOG_ERROR("entities.player", "%s has unexpectingly lost totem in slot {}!", me->GetName().c_str(), i);
        ...
        void DamageTaken(Unit* u, uint32& /*damage*/, DamageEffectType, SpellSchoolMask) override
        {
            if (!u)
                return;
        ...
        {
            //LOG_ERROR("entities.unit", "SummonedCreatureDespawn: %s's %s", me->GetName().c_str(), summon->GetName().c_str());
            //if (summon == botPet)
            //    botPet = nullptr;
            if (summon->GetEntry() == BOT_PET_SPIRIT_WOLF)
            {
        ...
        void UnsummonAll() override
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                if (_totems[i].first != ObjectGuid::Empty)
                {
                    ...
                    if (!to)
                    {
                        //LOG_ERROR("entities.player", "%s has no totem in slot {} during remove!", me->GetName().c_str(), i);
                        continue;
                    }
                    to->ToTotem()->UnSummon();
                }
        ...
        void OnBotDespawn(Creature* summon) override
        {
            if (!summon)
            {
                LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot %s received NULL", me->GetName().c_str());
                ASSERT(false);
                //UnsummonAll();
                return;
            }
            ...
            {
                //LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot %s has despawned summon %s which is not a temp summon or not a totem...", me->GetName().c_str(), summon->GetName().c_str());
                return;
            }
            ...
            switch (totem->m_Properties->Slot)
            {
                ...
                default:
                    //LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot %s has despawned totem %s in unknown slot {}", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->Id);
                    return;
            }

            if (_totems[slot].first == ObjectGuid::Empty)
                LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot %s has despawned totem %s while not having it registered!", me->GetName().c_str(), summon->GetName().c_str());
            else if (_totems[slot].second._type == BOT_TOTEM_NONE || _totems[slot].second._type >= BOT_TOTEM_END)
                LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot %s has despawned totem %s with no type assigned!", me->GetName().c_str(), summon->GetName().c_str());

            //here we reset totem category cd (not totem spell cd) if totem is destroyed
            //LOG_ERROR("entities.player", "OnBotDespawn(): %s despawned (%s : {})", summon->GetName().c_str(), summon->IsAlive() ? "alive" : summon->isDying() ? "justdied" : "unk", (uint32)summon->getDeathState());
        ...
        void OnBotSummon(Creature* summon) override
        {
            ...
            if (!totem || !totem->IsTotem())
            {
                //LOG_ERROR("entities.player", "OnBotSummon(): Shaman bot %s has summoned creature %s which is not a temp summon or not a totem...", me->GetName().c_str(), summon->GetName().c_str());
                return;
            }
            ...
            switch (totem->m_Properties->Slot)
            {
                ...
                case SUMMON_TYPE_TOTEM_AIR:     slot = T_AIR;   break;
                default:
                    //LOG_ERROR("entities.player", "OnBotSummon(): Shaman bot %s has summoned totem %s with unknown type {}", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->Id);
                    return;
            }
            ...
            //LOG_ERROR("entities.player", "shaman bot: summoned %s (type {}) at x=%.2f, y=%.2f, z=%.2f",
            ...
            summon->SetDisplayId(summon->GetModelForTotem(PlayerTotemType(btype)));
    ...
    private:
        ...
        uint32 _getTotemsMask(std::map<uint32 /*type*/, uint32 /*curId*/>& idMap) const
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = aurapps.begin(); itr != aurapps.end(); ++itr)
            {
                ...
                //{
                //    LOG_ERROR("spells","totemMask: unit %s, %s ({}), owner %s (crSp {}, base {}), istotem {}", target->GetName().c_str(),
