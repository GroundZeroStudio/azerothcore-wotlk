...
void DPSTracker::_Release()
{
    for (DamageTakenMap::const_iterator itr = _damages.begin(); itr != _damages.end(); ++itr)
    {
        ...
        //LOG_ERROR("entities.player", "DPSTracker::Release(): guidlow = {}, time = {}, tick damage {}, total {}, dps = {}",
...
{
    //LOG_ERROR("entities.player", "DPSTracker::OnDamage(): on %s, damage {}", victim->GetName().c_str(), damage);
...
uint32 DPSTracker::GetDPSTaken(uint64 guid) const
{
    ...
    //LOG_ERROR("entities.player", "DPSTracker::GetDPSTaken(): from {}, damage {}", guid, itr != _DPSes.end() ? itr->second : 0);
