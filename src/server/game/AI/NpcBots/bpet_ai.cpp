...
void bot_pet_ai::CureGroup(uint32 cureSpell, uint32 diff)
{
    ...
    if (!pGroup)
    {
        ...
        for (auto itr = petOwner->GetBotOwner()->m_Controlled.begin(); itr != petOwner->GetBotOwner()->m_Controlled.end(); ++itr)
        {
            Unit* u = *itr;
...
bool bot_pet_ai::_canCureTarget(Unit * target, uint32 cureSpell) const
{
...
void bot_pet_ai::SetPetStats(bool force)
{
    ...
    switch (petOwner->GetBotClass())
    {
        ...
        case BOT_CLASS_WARLOCK:
            ...
            //LOG_ERROR("entities.player", "SetPetStat(): atpower += 0.57 of %i = %.2f", spdtotal, atpower);
            break;
        case BOT_CLASS_DREADLORD:
            atpower += spdtotal * 6;
            //LOG_ERROR("entities.player", "SetPetStat(): atpower += 0.57 of %i = %.2f", spdtotal, atpower);
    ...
    //LOG_ERROR("entities.player", "SetPetStat(): hp stamval %.1f, stammult %.1f, base {}, total %.2f", stamValue, stamMult, botPet->GetCreateHealth(), m_totalhp);
    ...
    if (me->getPowerType() == POWER_MANA)
    {
        ...
        //LOG_ERROR("entities.player", "SetPetStat(): mana intValue %.1f, intMult %.1f, base {}, total %.2f", intValue, intMult, botPet->GetCreatePowerValue(POWER_MANA), m_totalmana);
...
bool bot_pet_ai::IsInBotParty(Unit const* unit) const
{
    ...
    //cheap check
    if (Group * gr = petOwner->GetBotOwner()->GetGroup())
    {
        //group member case
...
void bot_pet_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 10)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): count is out of bounds (%i) for bot %s (botclass: {}, entry: {})",
            int32(count), me->GetName().c_str(), uint32(petOwner->GetBotClass()), me->GetEntry());
        return;
    }
    if (!spellId)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): spellId is 0 for bot %s (botclass: {}, entry: {})",
    ...
    if (!spellInfo)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): Invalid spellInfo for spell {}! Bot - %s (botclass: {}, entry: {})",
...
void bot_pet_ai::MoveBehind(Unit const* target) const
{
    ...
    {
        float x,y,z;
        target->GetNearPoint(me, x, y, z, me->GetCombatReach(), 0.0f, me->GetAbsoluteAngle(target));
...
uint32 bot_pet_ai::InitSpell(Unit * caster, uint32 spell)
{
...
{
    //LOG_ERROR("entities.unit", "bot_pet_ai::IsSummonedBy for %s by %s", me->GetName().c_str(), summoner->GetName().c_str());
    //ASSERT(!petOwner);
...
void bot_pet_ai::OnBotPetSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotPetSpellMap::iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        //LOG_ERROR("entities.player", "OnBotPetSpellInterrupted(): Adding cooldown ({}, new: {}) to spell %s (id: {}, schoolmask: {}), reqSchoolMask = {}",
