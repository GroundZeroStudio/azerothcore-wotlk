...
{
    public:
        explicit HostileDispelTargetCheck(Unit * unit, float dist = 30, bool stealable = false, bot_ai const* m_ai = nullptr) :
    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = Auras.begin(); itr != Auras.end(); ++itr)
            {
                ...
                if (itr->first == spell)
                    if (!caster || auraApp->GetBase()->GetCasterGUID() == caster)
                        return true;
            }
            return false;
        }
...
{
    public:
        explicit PolyUnitCheck(Unit * unit, float dist) : me(unit), m_range(dist) {}
        bool operator()(Unit * u) const
        {
...
{
    public:
        explicit FearUnitCheck(Unit * unit, float dist = 30) : me(unit), m_range(dist) {}
        bool operator()(Unit * u) const
        {
            if (!_botPvP && !(me->ToCreature() && me->ToCreature()->IsFreeBot()) && u->IsControlledByPlayer())
                return false;
...
{
    public:
        explicit UndeadCCUnitCheck(Unit * unit, float dist, bot_ai const* ai, uint32 spell, bool unattacked) :
...
{
    public:
        explicit RootUnitCheck(Unit * unit, float dist, bot_ai const* ai, uint32 spell = 0) :
...
{
    public:
        explicit CastingUnitCheck(Unit * unit, float mindist = 0.f, float maxdist = 30, uint32 spell = 0, uint8 minHpPct = 0) :
...
{
    public:
        explicit TranquilTargetCheck(Unit * unit, float mindist, float maxdist, bot_ai const* m_ai) :
...
{
    public:
        explicit FarTauntUnitCheck(Unit * unit, float maxdist, bool ally, bot_ai const* m_ai) :
...
{
    public:
        explicit ManaDrainUnitCheck(Unit * unit, float maxdist) : me(unit), max_range(maxdist)
...
{
    public:
        explicit NearbyRezTargetCheck(Unit * unit, float maxdist, bot_ai const* m_ai) : me(unit), max_range(maxdist), ai(m_ai) { }
...
{
public:
    NearbyObjectBySkillCheck(WorldObject * checker, float const range, uint32 skillMask) :
    ...
    bool operator()(WorldObject const* ob)
    {
        ...
        else if (Creature const * ccre = ob->ToCreature())
        {
            Creature* cre = const_cast<Creature*>(ccre);
...
public:
    ...
    {
        if (Creature const* ccre = unit->ToCreature())
        {
            Creature* cre = const_cast<Creature*>(ccre);
