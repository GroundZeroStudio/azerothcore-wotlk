...
void BotDataMgr::LoadNpcBots(bool spawn)
{
    ...
    do
    {
        ...
        std::string _disabled_spells_str = field[++index].Get<std::string>();
        const char* disabled_spells_str = _disabled_spells_str.c_str();
    ...
    for (std::list<uint32>::const_iterator itr = entryList.begin(); itr != entryList.end(); ++itr)
    {
        ...
        if (!infores)
        {
            LOG_ERROR("server.loading", "Cannot spawn npcbot %s (id: {}), not found in `creature` table!", proto->Name.c_str(), entry);
            continue;
        }
        ...
        LOG_DEBUG("server.loading", "bot {}: spawnId {}, full %s", entry, tableGuid, Guid.ToString().c_str());
        ...
        //if (cEntry && cEntry->powerType < MAX_POWERS && uint32(bot->getPowerType()) != cEntry->powerType)
            //bot->setPowerType(Powers(cEntry->powerType));
        ...
        LOG_DEBUG("server.loading", ">> Spawned npcbot %s (id: {}, map: {}, grid: {}, cell: {})", proto->Name.c_str(), entry, mapId, g.GetId(), c.GetId());
        ...
        case NPCBOT_UPDATE_EQUIPS:
        {
            ...
            for (k = BOT_SLOT_MAINHAND; k != BOT_INVENTORY_SIZE; ++k)
            {
                ...
                if (Item const* botitem = items[k])
                {
                    ...
                    stmt->SetData(++index, botitem->GetItemRandomPropertyId());
                    stmt->SetData(++index, botitem->GetUInt32Value(ITEM_FIELD_DURABILITY));
...
void BotDataMgr::RegisterBot(Creature const* bot)
{
    ...
    {
        LOG_ERROR("entities.unit", "BotDataMgr::RegisterBot: bot {} (%s) already registered!",
    ...
    //LOG_ERROR("entities.unit", "BotDataMgr::RegisterBot: registered bot {} (%s)", bot->GetEntry(), bot->GetName().c_str());
}
void BotDataMgr::UnregisterBot(Creature const* bot)
{
    if (_existingBots.find(bot) == _existingBots.end())
    {
        LOG_ERROR("entities.unit", "BotDataMgr::UnregisterBot: bot {} (%s) not found!",
            bot->GetEntry(), bot->GetName().c_str());
        return;
    }
    ...
    //LOG_ERROR("entities.unit", "BotDataMgr::UnregisterBot: unregistered bot {} (%s)", bot->GetEntry(), bot->GetName().c_str());
}
