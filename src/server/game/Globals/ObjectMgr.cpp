...
void ObjectMgr::LoadCreatureOutfits()
{
	...
	do
	{
		...
		if (!GetCreatureTemplate(entry))
		{
			LOG_ERROR("server.loading", ">> Creature entry %u in `creature_template_outfits`, but not in `creature_template`!", entry);
			continue;
		}
		...
		if (!rEntry)
		{
			LOG_ERROR("server.loading", ">> Creature entry %u in `creature_template_outfits` has incorrect race (%u).", entry, uint32(co.race));
			continue;
		}
		...
		case GENDER_MALE:
			...
			break;
		default:
			LOG_ERROR("server.loading", ">> Creature entry %u in `creature_template_outfits` has invalid gender %u", entry, uint32(co.gender));
			continue;
		}
